...
TZ_ELDOR = pytz.timezone("Asia/Tashkent")
TZ_RANO  = pytz.timezone("Asia/Seoul")
DAILY_QUESTION_HOUR = 20

supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)

MOOD_ELDOR, MOOD_RANO = range(2)
JOURNAL_TITLE, JOURNAL_BODY, JOURNAL_TAG = range(3)
QUIZ_ANSWER, QUIZ_GUESS = range(2)
GOAL_SELECT, GOAL_WHO, GOAL_TEXT = range(3)
...
GOAL_CATEGORIES = [
    ("career","💼 Career & Purpose"),("family","🕌 Family & Marriage"),
    ("location","🌍 Where We Live"),("faith","🤲 Faith & Values"),
    ("finance","📈 Financial Goals"),("growth","🌱 Personal Growth"),
]

QUIZ_QUESTIONS = [
    ("What's my comfort food when I'm stressed?","rano"),
    ("Which city would I most want to live in for 1 year?","eldor"),
    ("What's my go-to way to relax after a hard day?","rano"),
    ("What am I secretly most proud of?","eldor"),
    ("What's my biggest irrational fear?","rano"),
    ("What do I find most attractive in a partner?","eldor"),
    ("What habit of mine do I wish I could change?","rano"),
    ("What's the first thing I notice about a person?","eldor"),
    ("What kind of music do I listen to when I'm emotional?","rano"),
    ("What's my love language?","eldor"),
]

def who(username):
    if username and username.lower() == ELDOR_USERNAME.lower(): return "eldor"
    if username and username.lower() == RANO_USERNAME.lower(): return "rano"
    return "unknown"

def display_name(w): return "Eldor" if w == "eldor" else "Ra'no"
def other(w): return "rano" if w == "eldor" else "eldor"
...
def db_set(table, key, data):
    try:
        ...
        if existing: supabase.table(table).update({"data": data}).eq("key", key).execute()
        else: supabase.table(table).insert({"key": key, "data": data}).execute()
...
async def start(update, ctx):
    ...
    text = (f"🐧💛🐧 *Welcome to Ra'El, {name}*\n\n_Just the two of us — Eldor & Ra'no_\n\n"
            f"🎲 /question — Random question\n🎯 /quiz — Couples quiz\n📊 /mood — Log today's mood\n"
            f"📖 /journal — Add a memory\n🌍 /goals — Life goals\n📈 /moodchart — Mood history\n"
            f"🗂 /memories — Browse journal\n💛 /us — Relationship stats\n")
    await update.message.reply_text(text, parse_mode="Markdown")

async def question(update, ctx):
    keyboard = [[InlineKeyboardButton(t, callback_data=f"topic:{t}")] for t in QUESTIONS]
...
async def topic_callback(update, ctx):
    ...
    await query.answer()
    topic = query.data.split("topic:")[1]
...
async def mood_cmd(update, ctx):
    ...
    w = who(username)
    if w == "unknown":
        await update.message.reply_text("⚠️ I don't recognize your username.")
        return ConversationHandler.END
    today = date.today().isoformat()
    existing = db_get_data("moods", today) or {}
    keyboard = [[InlineKeyboardButton(e, callback_data=f"mood:{s}:{w}") for s,e in [("1","😔"),("2","😕"),("3","😊"),("4","🥰"),("5","✨")]]]
    msg = f"You already logged *{MOODS[str(existing[w])]}* today. Update it?" if w in existing else f"*{display_name(w)}, how are you feeling today?*"
...
async def mood_callback(update, ctx):
    ...
    if other_score:
        ...
        sync = "💛 Perfectly in sync!" if diff==0 else "🌊 Almost in sync." if diff==1 else "💙 You're in different places. Reach out." if diff>=3 else "😊 Slightly different vibes, that's okay."
        text = f"*Mood logged* ✓\n\nEldor: {MOODS[str(existing.get('eldor',score))]}\nRa'no: {MOODS[str(existing.get('rano',score))]}\n\n{sync}"
    else:
        text = f"*{MOODS[str(score)]}* — logged ✓\n\nWaiting for {display_name(other(w))} to log their mood..."
    await query.edit_message_text(text, parse_mode="Markdown")

async def moodchart(update, ctx):
    try: data = supabase.table("moods").select("*").order("key",desc=True).limit(14).execute().data
    except: data = []
    if not data:
        await update.message.reply_text("No mood data yet. Use /mood to start."); return
    lines = ["*📊 Last 14 days*\n"]
    for row in reversed(data):
        d = row["data"]
        e = MOODS.get(str(d.get("eldor")),"—") if d.get("eldor") else "—"
        r = MOODS.get(str(d.get("rano")),"—") if d.get("rano") else "—"
        lines.append(f"`{row['key'][5:]}` E: {e}  R: {r}")
...
async def journal_body(update, ctx):
    ...
    keyboard = [[InlineKeyboardButton(t, callback_data=f"tag:{t}") for t in ["First","Milestone","Favorite"]],
                [InlineKeyboardButton(t, callback_data=f"tag:{t}") for t in ["Funny","Deep","Everyday"]]]
...
async def journal_tag(update, ctx):
    query = update.callback_query
    await query.answer()
    tag = query.data.split("tag:")[1]
    w = who(update.effective_user.username or "")
    entry = {"id":int(datetime.now().timestamp()),"title":ctx.user_data.get("journal_title",""),
             "body":ctx.user_data.get("journal_body",""),"tag":tag,"by":w,
             "date":date.today().strftime("%-d %b %Y"),"ts":int(datetime.now().timestamp())}
    existing = db_get_data("journal","entries") or []
    existing.insert(0, entry)
    db_set("journal","entries",existing)
    await query.edit_message_text(f"📖 *Memory saved* ✓\n\n*{entry['title']}*\n_{entry['body'][:100]}_\n\nTagged: {tag} · By {display_name(w)}", parse_mode="Markdown")
    return ConversationHandler.END

async def memories(update, ctx):
    entries = db_get_data("journal","entries") or []
    if not entries:
        await update.message.reply_text("No memories yet. Use /journal to add your first."); return
    lines = [f"📖 *{len(entries)} memories*\n"]
    for e in entries[:8]:
        by = "E" if e.get("by")=="eldor" else "R"
        lines.append(f"[{by}] *{e['title']}* · {e.get('tag','')} · {e.get('date','')}")
        if e.get("body"): lines.append(f"_{e['body'][:60]}{'...' if len(e['body'])>60 else ''}_")
        lines.append("")
    await update.message.reply_text("\n".join(lines), parse_mode="Markdown")

async def goals_cmd(update, ctx):
    keyboard = [[InlineKeyboardButton(l, callback_data=f"goalcat:{c}")] for c,l in GOAL_CATEGORIES]
    ...
    if query.data == "goalcat:view":
        ...
        for cat_id, label in GOAL_CATEGORIES:
            ...
            if g.get("eldor") or g.get("rano"):
                ...
                if g.get("eldor"): lines.append(f"  Eldor: {g['eldor']}")
                if g.get("rano"):  lines.append(f"  Ra'no: {g['rano']}")
                if g.get("note"):  lines.append(f"  📌 {g['note']}")
                lines.append("")
        await query.edit_message_text("\n".join(lines) or "No goals yet.", parse_mode="Markdown"); return
    cat_id = query.data.split("goalcat:")[1]
    ctx.user_data["goal_cat"] = cat_id
    label = next(l for c,l in GOAL_CATEGORIES if c==cat_id)
    g = (db_get_data("goals","main") or {}).get(cat_id,{})
    existing_text = ""
    if g.get("eldor"): existing_text += f"\nEldor: _{g['eldor']}_"
    if g.get("rano"):  existing_text += f"\nRa'no: _{g['rano']}_"
    keyboard = [[InlineKeyboardButton("Eldor's view",callback_data="goalwho:eldor"),InlineKeyboardButton("Ra'no's view",callback_data="goalwho:rano")],
                [InlineKeyboardButton("Shared note",callback_data="goalwho:note")]]
    await query.edit_message_text(f"*{label}*{existing_text}\n\nWhose perspective?", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard))

async def goalwho_callback(update, ctx):
    query = update.callback_query
    await query.answer()
    w = query.data.split("goalwho:")[1]
    ctx.user_data["goal_who"] = w
    label = "Eldor" if w=="eldor" else ("Ra'no" if w=="rano" else "shared note")
    await query.edit_message_text(f"Type {label}'s perspective:")
    return GOAL_TEXT
...
async def goal_text_handler(update, ctx):
    ...
    goals = db_get_data("goals","main") or {}
    if cat_id not in goals: goals[cat_id] = {}
    goals[cat_id][w] = update.message.text
    db_set("goals","main",goals)
    label = next(l for c,l in GOAL_CATEGORIES if c==cat_id)
    await update.message.reply_text(f"✓ Saved under *{label}*", parse_mode="Markdown")
    return ConversationHandler.END

async def quiz_cmd(update, ctx):
    ctx.user_data.update({"quiz_answers":[],"quiz_idx":0,"quiz_phase":"answer"})
    q, w = QUIZ_QUESTIONS[0]
    await update.message.reply_text(f"🎯 *Couples Quiz*\n\n*{display_name(w)}* — answer this about yourself:\n\n_{q}_\n\n_(1/{len(QUIZ_QUESTIONS)})_", parse_mode="Markdown")
    return QUIZ_ANSWER
...
async def quiz_answer(update, ctx):
    ...
    if idx < len(QUIZ_QUESTIONS):
        ...
        await update.message.reply_text(f"*{display_name(w2)}* — answer this:\n\n_{q2}_\n\n_({idx+1}/{len(QUIZ_QUESTIONS)})_", parse_mode="Markdown")
        return QUIZ_ANSWER
    ctx.user_data.update({"quiz_idx":0,"quiz_guesses":[],"quiz_score":0})
    a = ctx.user_data["quiz_answers"][0]
    await update.message.reply_text(f"✅ Done! Now *{display_name(other(a['who']))}* guesses.\n\n*{display_name(a['who'])}* was asked:\n_{a['q']}_\n\nWhat did they say?", parse_mode="Markdown")
    return QUIZ_GUESS
...
async def quiz_guess(update, ctx):
    ...
    close = guess.lower() in correct or correct in guess.lower() or any(w in correct for w in guess.lower().split() if len(w)>3)
    if close: ctx.user_data["quiz_score"] += 1
    ctx.user_data["quiz_guesses"].append({**current,"guess":guess,"correct":close})
    idx += 1
    ctx.user_data["quiz_idx"] = idx
    if idx < len(answers):
        a = answers[idx]
        await update.message.reply_text(f"{'✅ Close!' if close else f'❌ They said: _{current[\"answer\"]}_'}\n\n*{display_name(other(a['who']))}* — what did *{display_name(a['who'])}* say?\n\n_{a['q']}_", parse_mode="Markdown")
        return QUIZ_GUESS
    score = ctx.user_data["quiz_score"]
    pct = round(score/len(answers)*100)
    emoji = "🥰" if pct>=80 else "😊" if pct>=50 else "🌱"
    msg = "You know each other deeply 💛" if pct>=80 else "Pretty well — keep learning 🌊" if pct>=50 else "Room to grow 🌱"
    lines = [f"{emoji} *{pct}% — {msg}*\n"]
    for g in ctx.user_data["quiz_guesses"]:
        lines += [f"{'✅' if g['correct'] else '❌'} _{g['q']}_", f"  Answer: {g['answer']}", f"  Guess: {g['guess']}\n"]
    await update.message.reply_text("\n".join(lines), parse_mode="Markdown")
    return ConversationHandler.END
...
async def us(update, ctx):
    try:
        ...
        entries = db_get_data("journal","entries") or []
        goals = db_get_data("goals","main") or {}
    except: mood_rows, entries, goals = [], [], {}
    days = len(mood_rows)
    filled = sum(1 for g in goals.values() if g.get("eldor") and g.get("rano"))
    if mood_rows:
        ae = sum(r["data"].get("eldor",0) for r in mood_rows if r["data"].get("eldor"))/max(1,sum(1 for r in mood_rows if r["data"].get("eldor")))
        ar = sum(r["data"].get("rano",0) for r in mood_rows if r["data"].get("rano"))/max(1,sum(1 for r in mood_rows if r["data"].get("rano")))
        mood_line = f"Eldor avg: {ae:.1f}/5 · Ra'no avg: {ar:.1f}/5"
    else: mood_line = "No mood data yet"
    await update.message.reply_text(f"🐧💛🐧 *Eldor & Ra'no*\n\n📊 Mood days: *{days}*\n_{mood_line}_\n\n📖 Memories: *{len(entries)}*\n🌍 Goals aligned: *{filled}/{len(GOAL_CATEGORIES)}*\n\n_Just the two of you. Keep going._ 💛", parse_mode="Markdown")
...
def main():
    ...
    app.add_handler(ConversationHandler(
        ...
        states={JOURNAL_TITLE:[MessageHandler(filters.TEXT&~filters.COMMAND,journal_title)],
    ...
    app.add_handler(ConversationHandler(
        ...
        states={QUIZ_ANSWER:[MessageHandler(filters.TEXT&~filters.COMMAND,quiz_answer)],
    ...
    app.add_handler(ConversationHandler(
        ...
        states={GOAL_TEXT:[MessageHandler(filters.TEXT&~filters.COMMAND,goal_text_handler)]},
        fallbacks=[CommandHandler("cancel",cancel)]))
